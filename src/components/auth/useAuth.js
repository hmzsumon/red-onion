...
const UserProvider = (props) => {
  ...
  const login = async (email, password) => {
    try {
      ...
      const userInfo = await axios.get(
        `https://red-onion03.herokuapp.com/api/users/email/${email}`
      );
  ...
  const registerUserWithEmailPassword = async (email, password, name) => {
    try {
      ...
      const createdUeser = await axios.post(
        'https://red-onion03.herokuapp.com/api/users',
        {
          email,
          displayName: name,
        }
  ...
  const calculateQuantity = (item, event) => {
    ...
    if (product.quantity === 0) {
      ...
      // delete product
      axios.delete(
        `https://red-onion03.herokuapp.com/api/carts/${item._id}?user=${user._id}`
      );
      setCart(updateProduct);
    } else {
      // update product
      axios.put(`https://red-onion03.herokuapp.com/api/carts/${item._id}`, {
  ...
  function addCatDatabase(item, quantity, user) {
    ...
    axios.post('https://red-onion03.herokuapp.com/api/carts/', cartData);
  }
